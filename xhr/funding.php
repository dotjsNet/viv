    	...
    	if (!empty($_GET['amount']) && is_numeric($_GET['amount']) && $_GET['amount'] > 0 && !empty($_GET['fund_id']) && is_numeric($_GET['fund_id']) && $_GET['fund_id'] > 0) {
			...
			if (!empty($fund)) {
				if ($amount <= $left) {
                    ...
                    $html = str_replace('{company_address}', $companyAddr, $html);
                    $html = str_replace('{vat_num}', $vatNum, $html);
					$data['status'] = 200;
					$data['html'] = $html;
				}
				else{
    ...
    if ($s == 'paypal_paid' && $wo['config']['paypal'] != 'no' && !empty($_GET['fund_id']) && !empty($_GET['amount'])) {
    	...
    	if (!empty($fund) && !empty($fund_id) && !empty($amount) ) {
		    ...
		    try {
	            ...
	            $id = Wo_RegisterPost($post_data);
                //create invoice
                $invoiceID  = Wo_InvoiceData($wo['user']['user_id'],"fund",$amount,0,$amount,$notes,1);
    ...
    if ($s == 'stripe' && $wo['config']['credit_card'] != 'no' && !empty($_POST['fund_id']) && is_numeric($_POST['fund_id']) && $_POST['fund_id'] > 0 && !empty($_POST['amount']) && is_numeric($_POST['amount']) && $_POST['amount'] > 0) {
	    ...
	    try {
	        ...
	        if ($charge) {
                ...
                //create invoice
                $invoiceID  = Wo_InvoiceData($wo['user']['user_id'],"fund",$amount,0,$amount,$notes,1,$charge->id,$charge->source->customer,$charge->source->name);
                $html       = WoGenerateInvoiceHTML($invoiceID);
                $file       = Wo_GeneratePDF($invoiceID,$html);
                $send_message_data       = array(
                    'from_email'    => ($wo['config']['invoice_email'])?$wo['config']['invoice_email']:$wo['config']['siteEmail'],
        ...
        if (empty($error) && !empty($fund)) {
            ...
            if (!empty($mediaFilename)) {
                $insert_id = Wo_InsertBankTrnsfer(array('user_id' => $wo['user']['id'],
                                                       ...
                                                       'fund_id' => $fund_id));
                //create invoice
                $invoiceID  = Wo_InvoiceData($wo['user']['user_id'],"fund",$amount,0,$amount,$description,1);
                $html       = WoGenerateInvoiceHTML($invoiceID);
                $file       = Wo_GeneratePDF($invoiceID,$html);
                $send_message_data       = array(
                    'from_email'    => ($wo['config']['invoice_email'])?$wo['config']['invoice_email']:$wo['config']['siteEmail'],
	        ...
	        if (!empty($fund)) {
		        ...
		        try {
		            ...
		            if ($charge['response']['responseCode'] == 'APPROVED') {
		                ...
		                Wo_RegisterNotification($notification_data_array);
                        //create invoice
                        $invoiceID  = Wo_InvoiceData($wo['user']['user_id'],"funding",$amount,0,$amount,$notes,1);
                        $html       = WoGenerateInvoiceHTML($invoiceID);
                        $file       = Wo_GeneratePDF($invoiceID,$html);
                        $send_message_data       = array(
                            'from_email'    => ($wo['config']['invoice_email'])?$wo['config']['invoice_email']:$wo['config']['siteEmail'],
