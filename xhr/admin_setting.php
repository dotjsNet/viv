...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'remove_invoice') {
        if (!empty($_POST['invoiceID'])) {
            $file =  $db->where('id',Wo_Secure($_POST['invoiceID']))->getOne(T_INVOICES);
            unlink($file->file);
            $db->where('id',Wo_Secure($_POST['invoiceID']))->delete(T_INVOICES);
        }
        $data = array(
            'status' => 200
        );
    ...
    if ($s == 'get_download_link') {
        $data = array(
            'status' => 400
        );
        if(count($_REQUEST['invoices'])>0){
            $invoices = $db->where('id', $_REQUEST['invoices'],'in' )->get(T_INVOICES, null, 'id');
            if(count($invoices) > 0){
                $downloadUri = [];
                foreach ($invoices as $invoice){
                    $downloadUri[]=$invoice->id;
                }
                $data = [ 'status' => 200, 'url' => '/download.php?ids='.implode(',',$downloadUri).'&hash='.$_SESSION['hash_id']];
            }
        }else if($_REQUEST['month'] > 0){
            $month      = Wo_Secure($_REQUEST['month']);
            $query = 'select id from '.T_INVOICES.' where created_at >= "'.date('Y').'-'.$month.'-1" and created_at <= "'.date('Y').'-'.$month.'-31"';
            $invoices   = $db->rawQueryOne($query );
            if($invoices){
                $data = [ 'status' => 200, 'url' => '/download.php?month='.$_REQUEST['month'].'&hash='.$_SESSION['hash_id']];
            }
        }
        header("Content-type: application/json");
        echo json_encode($data);
