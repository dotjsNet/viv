			    ...
			    if ($stop == 0) {
			        ...
			        if ($is_pro == 1) {
			            ...
			            if ($pro_type == 1) {
                            ...
                            $proTitle  = $wo['lang']['star'].' '.$wo['lang']['member'];
			            } else if ($pro_type == 2) {
			                $img     = $wo['lang']['hot'];
			                $amount1 = $wo['pro_packages']['hot']['price'];
                            $proTitle  = $wo['lang']['hot'].' '.$wo['lang']['member'];
			            } else if ($pro_type == 3) {
			                $img     = $wo['lang']['ultima'];
			                $amount1 = $wo['pro_packages']['ultima']['price'];
                            $proTitle  = $wo['lang']['ultima'].' '.$wo['lang']['member'];
			            } else if ($pro_type == 4) {
			                $img     = $wo['lang']['vip'];
			                $amount1 = $wo['pro_packages']['vip']['price'];
                            $proTitle  = $wo['lang']['vip'].' '.$wo['lang']['member'];
			            }
			            ...
			            if ($mysqli) {
                            //create invoice
                            $invoiceID  = Wo_InvoiceData($wo['user']['user_id'],$proTitle,$amount1,$pro_type,$amount1,"New ".$proTitle." badge is bought",1);
    	...
    	if (!empty($_POST['type']) && $_POST['type'] == 'wallet' && !empty($_POST['payment_id']) && !empty($_POST['order_id']) && !empty($_POST['merchant_amount']) && !empty($_POST['currency'])) {
			...
			if (!empty($json) && empty($json->error_code)) {
				...
				if (Wo_ReplenishingUserBalance($price)) {
                    ...
                    //create invoice
                    $invoiceID  = Wo_InvoiceData($wo['user']['user_id'],"wallet",$price,0,$price,"wallet account has been recharged",1);
                    $html       = WoGenerateInvoiceHTML($invoiceID);
                    $file       = Wo_GeneratePDF($invoiceID,$html);
                    $send_message_data       = array(
                        'from_email'    => ($wo['config']['invoice_email'])?$wo['config']['invoice_email']:$wo['config']['siteEmail'],
    	...
    	if (!empty($_POST['type']) && $_POST['type'] == 'fund' && !empty($_POST['payment_id']) && !empty($_POST['order_id']) && !empty($_POST['merchant_amount']) && !empty($_POST['currency'])) {
			...
			if (!empty($json) && empty($json->error_code)) {
		    	...
		    	if (!empty($fund) && !empty($fund_id) && !empty($amount)) {
                    ...
                    //create invoice
                    $invoiceID  = Wo_InvoiceData($wo['user']['user_id'],"funding",$amount+$admin_com,0,$amount+$admin_com,$notes,1);
                    $html       = WoGenerateInvoiceHTML($invoiceID);
                    $file       = Wo_GeneratePDF($invoiceID,$html);
                    $send_message_data       = array(
                        'from_email'    => ($wo['config']['invoice_email'])?$wo['config']['invoice_email']:$wo['config']['siteEmail'],
