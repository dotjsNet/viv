...
function Wo_InvoiceData($userid,$title,$price,$productID,$totalPrice,$desc,$qty,$invoiceNum = null,$payerName=null,$payerEmail=null){
    global $wo,$sqlConnect;
    list($companyName, $companyAddr, $vatNum) = explode(',',$_COOKIE['printMeta']);
    $addQueryField = "";
    $addQueryValue = "";
    if($invoiceNum != ''){
        $addQueryField .= ", `invoice_num`";
        $addQueryValue .= ", '$invoiceNum'";
    }
    if($payerName != ''){
        $addQueryField .= ", `invoice_name`";
        $addQueryValue .= ", '$payerName'";
    }
    if($payerEmail != ''){
        $addQueryField .= ", `invoice_email`";
        $addQueryValue .= ", '$payerEmail'";
    }
    $insertQ = "INSERT INTO ".T_INVOICES." (`user_id`,`product_title`,`product_price`, `product_id`,`total_price`,`desc`,`qty`,`vat`,`file`,`company_name`,`company_address`,`vat_number`".$addQueryField.") VALUES ('$userid','$title','$price','$productID','$totalPrice','$desc','$qty','{$wo[config][vat_rate]}','','$companyName', '$companyAddr', '$vatNum'".$addQueryValue.")";
    mysqli_query($sqlConnect, $insertQ);
    return mysqli_insert_id($sqlConnect);
}
function Wo_GeneratePDF($invoiceID = null,$html){
    global $wo,$sqlConnect;
    if(!$invoiceID) return false;
    require_once __DIR__ . '/../libraries/pdfgenerator/autoload.php';
    if (!file_exists('upload/invoice/' . date('Y'))) {
        @mkdir('upload/invoice/' . date('Y'), 0777, true);
    }
    if (!file_exists('upload/invoice/' . date('Y') . '/' . date('m'))) {
        @mkdir('upload/invoice/' . date('Y') . '/' . date('m'), 0777, true);
    }
    $dir         = "upload/invoice/" . date('Y') . '/' . date('m');
    $filename    = $dir . '/' . Wo_GenerateKey() . '_' . date('d') . '_' . md5(time()) . "_invoice.pdf";

    $mpdf = new \Mpdf\Mpdf();
    $mpdf->WriteHTML($html);
    $mpdf->Output($filename,\Mpdf\Output\Destination::FILE);
    $query = "UPDATE ".T_INVOICES." SET file = '".$filename."' where id = ".$invoiceID;
    mysqli_query($sqlConnect, $query);
    return $filename;
}
function WoGenerateInvoiceHTML($invoiceID = null){
    global $wo,$sqlConnect;
    if(!$invoiceID) return false;
    $_REQUEST['invoice_id'] = $invoiceID;
    ob_start();
